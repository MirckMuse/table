...
export class TableRowStateCenter {
	...
	async getFilteredRowDatas(rowDatas: RowData[]): Promise<RowData[]> {
					...
					if (filterKeys?.length) {
						return filteredRows.filter((row) => {
							return filterKeys.some(
								(key: string) => row.data[dataIndex] === key,
							);
						});
					}
		...
		const _task = (rows: RowData[]) => {
			// FIXME: 10w 条数据消耗 20ms, 希望可以优化到 10ms 以内
			const start = performance.now();
			const _createGeneratePoolRows = () => {
				if (this.getRowKey) {
					return (rows: RowData[]): PoolRow[] => {
			...
			const rawFilterStates = toRaw(this.filterStates).map((state) =>
				Object.assign(
					{
						dataIndex: colStateCenter.getColumnByColKey(state.colKey)
							?.dataIndex,
					},
					state,
				),
		...
		try {
			const rows = await _task(rowDatas);
			return rows.reduce<RowData[]>((result, key) => {
